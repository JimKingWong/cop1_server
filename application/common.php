...
if (!function_exists('cdnurl')) {
    ...
    function cdnurl($url, $domain = false)
    {
        ...
        $env = is_local_env() ? 'localcdnurl' : 'cdnurl';
        $cdnurl = \think\Config::get('upload.' . $env);
...
function getQrcode($url)
{
    ...
    ob_start();
    $qrcode = new \QRcode();
    $qrcode::png($url, null, QR_ECLEVEL_L, 10, 4);
    $imgageData = ob_get_contents();
    ob_end_clean();
    return 'data:image/png;base64,' . base64_encode($imgageData);
}
 ...
 * 生成代理和用户的邀请码
 */
function createInviteCode($num = 8)
{
    return strtolower(substr(md5(uniqid(mt_rand(), true)), 0, $num));
}
