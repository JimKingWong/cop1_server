...
class Config extends Model
{
    ...
    {
        $groupList = config('dictionary.configgroup');
     ...
     * 刷新配置文件
     */
    public static function refreshFile($custom_group)
    {
        //如果没有配置权限无法进行修改
        ...
        // 根据分组名, 创建配置文件
        if($custom_group != 'basic'){
            $filename = $custom_group;
        }

        $configList = self::where('group', $custom_group)->select();
